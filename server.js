...
const { askGemini, askGeminiWithMedia, askMalvryx, generateGeminiImage } = require('./utils/ai.js');
const historyManager = require('./utils/history.js');
const workspace = require('./utils/workspace.js');
const terminal = require('./utils/terminal.js');
const stickerPack = require('./utils/stickerPack.js');
const agentTools = require('./tools.js');
const { handleGitPushText, startGitPush } = require('./scenes/gitPush.js');
const { buildHelpText } = require('./commands/help.js');
const accessControl = require('./utils/accessControl.js');
const { appendLog, tailLogs } = require('./utils/logs.js');
const { requestWithRetry } = require('./utils/httpRetry.js');
const consoleCapture = require('./utils/consoleCapture.js');
const exec = require('./utils/executor.js');
const { isZipFileName, listWorkspaceZips, saveZipUpload, unzipFile } = require('./utils/fileHandler.js');
const { uploadToImgBB } = require('./utils/imgbb.js');
const movieAPI = require('./utils/movieAPI.js');
const tmdbAPI = require('./utils/tmdbAPI.js');
// const publicDomainAPI = require('./utils/publicDomainAPI'); // FILE NOT FOUND - commented out
const apkDownloader = require('./apkDownloader.js');
const security = require('./utils/security.js');
const CodeFormatter = require('./codeFormatter.js'); // FIXED: was in utils/ but file is in root
const PollSystem = require('./pollSystem.js'); // FIXED: was in utils/ but file is in root
// const MessageFormatter = require('./utils/MessageFormatter'); // FILE NOT FOUND - commented out
const imageAI = require('./imageAI.js'); // FIXED: was in utils/ but file is in root
const voiceCloner = require('./utils/voiceCloner.js');
...
async function handlePublicDomainDownload(ctx, query) {
  ...
  try {
//     const results = await publicDomainAPI.searchMovies(query, 5); // COMMENTED: publicDomainAPI file not found
    if (!results.length) {
...
//     const fileInfo = await publicDomainAPI.getBestDownloadUrl(targetMovie.id); // COMMENTED: publicDomainAPI file not found
    if (!fileInfo) {
      await ctx.telegram.editMessageText(ctx.chat.id, progressMsg.message_id, undefined,
    ...
    if (fileInfo.size > TELEGRAM_LIMIT) {
      await ctx.telegram.editMessageText(ctx.chat.id, progressMsg.message_id, undefined,
//         `⚠️ File too large to send via Telegram (${publicDomainAPI.formatBytes(fileInfo.size)}).\n\n🔗 Direct download link:\n${fileInfo.url}\n\n📄 Archive page: ${targetMovie.url}`); // COMMENTED: publicDomainAPI file not found
      return;
    }

    await ctx.telegram.editMessageText(ctx.chat.id, progressMsg.message_id, undefined,
//       `⬇️ Downloading *${targetMovie.title}*...\n📦 Size: ${publicDomainAPI.formatBytes(fileInfo.size)}\n⏳ 0%`, { parse_mode: 'Markdown' }); // COMMENTED: publicDomainAPI file not found

    let lastPercent = 0;
//     const downloadResult = await publicDomainAPI.downloadMovie(targetMovie.id, (percent) => { // COMMENTED: publicDomainAPI file not found
      if (percent >= lastPercent + 25) {
        lastPercent = percent;
        ctx.telegram.editMessageText(ctx.chat.id, progressMsg.message_id, undefined,
//           `⬇️ Downloading *${targetMovie.title}*...\n📦 Size: ${publicDomainAPI.formatBytes(fileInfo.size)}\n⏳ ${percent}%`, { parse_mode: 'Markdown' }).catch(() => {}); // COMMENTED: publicDomainAPI file not found
      }
    });

    await ctx.telegram.editMessageText(ctx.chat.id, progressMsg.message_id, undefined,
//       `✅ Download complete!\n📁 ${downloadResult.filename}\n📦 ${publicDomainAPI.formatBytes(downloadResult.size)}${downloadResult.cached ? ' (cached)' : ''}\n\n📤 Sending via Telegram...`, { parse_mode: 'Markdown' }); // COMMENTED: publicDomainAPI file not found
...
🎬 *TMDB Commands (Recent Movies):*\n/tmdb <name> - Search recent movies\n/nowplaying - Movies in theaters now\n/popular - Popular movies\n/toprated - Top rated movies`;
  ...
  if (command === 'pdmovie' || command === 'pdsearch') {
    ...
    try {
//       const results = await publicDomainAPI.searchMovies(args, 8); // COMMENTED: publicDomainAPI file not found
...
//         const formatted = publicDomainAPI.formatSearchResults(results, 5); // COMMENTED: publicDomainAPI file not found
...
//       return ctx.reply(MessageFormatter.reformatResponse(msg)); // COMMENTED: MessageFormatter file not found
    }
    if (result.type === 'images' && Array.isArray(result.images)) {
//       await ctx.reply(MessageFormatter.reformatResponse(`✅ Generated ${result.images.length} image(s) for: ${result.prompt || 'your prompt'}`)); // COMMENTED: MessageFormatter file not found
      for (const image of result.images.slice(0, 10)) {
        const imageUrl = image.url || image.path || image;
        try {
//           await ctx.replyWithPhoto(image.path ? { source: image.path } : imageUrl, { caption: MessageFormatter.reformatResponse(`🖼️ ${result.prompt || 'Generated image'}${image.seed ? ` // COMMENTED: MessageFormatter file not found
Seed: ${image.seed}` : ''}`) });
        } catch (_error) {
...
//       await ctx.reply(MessageFormatter.reformatResponse(`✅ Done. Scrape saved: ${result.savedPath} // COMMENTED: MessageFormatter file not found
...
//       await ctx.reply(MessageFormatter.reformatResponse(`✅ Done. File created: ${result.path}`)); // COMMENTED: MessageFormatter file not found
      if (isImage) {
...
//     return ctx.reply(MessageFormatter.reformatResponse(`✅ ${jsonStr}`)); // COMMENTED: MessageFormatter file not found
  }
  const textResult = String(result || 'Done').slice(0, 3500);
//   return ctx.reply(MessageFormatter.reformatResponse(textResult)); // COMMENTED: MessageFormatter file not found
}
...
async function runAgnesFallbackAgent(userMsg, history = [], sendFeedback, userId, depth = 0) {
  ...
  const { askAgnes } = require('./utils/ai.js');
...
bot.start(async (ctx) => {
  ...
  const welcomeCaption = `✅ *Bot connected successfully!*\n\nYo! I'm *OMNI* — your all-in-one AI assistant. Here's what I can do for you:\n\n💬 *AI Chat* — Ask me anything\n🖼️ *Generate Images* — Create & edit photos\n🎵 *Music & Video* — Download & play\n🎬 *Movies* — Search & stream links\n💻 *Terminal* — Run commands & code\n🌐 *Web Tools* — Scrape & screenshots\n📱 *APK Download* — Android apps\n🗳️ *Polls* — Create votes\n\n👇 Just chat naturally or use /help for all commands!`;
...
//     console.log('📝 MessageFormatter loaded'); // COMMENTED: MessageFormatter file not found
  })
  .catch((error) => {
