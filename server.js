...
const publicDomainAPI = require('./utils/publicDomainAPI');
const apkDownloader = require('./apkDownloader');
const security = require('./utils/security');
const CodeFormatter = require('./utils/CodeFormatter');
const PollSystem = require('./utils/PollSystem');
const MessageFormatter = require('./utils/MessageFormatter');
const imageAI = require('./utils/imageAI');
const voiceCloner = require('./utils/voiceCloner');
...
async function handlePublicDomainDownload(ctx, query) {
  ...
  try {
    const results = await publicDomainAPI.searchMovies(query, 5);
    if (!results.length) {
    ...
    const fileInfo = await publicDomainAPI.getBestDownloadUrl(targetMovie.id);
    if (!fileInfo) {
      await ctx.telegram.editMessageText(ctx.chat.id, progressMsg.message_id, undefined,
...
🎬 *TMDB Commands (Recent Movies):*\n/tmdb <name> - Search recent movies\n/nowplaying - Movies in theaters now\n/popular - Popular movies\n/toprated - Top rated movies`;
  ...
  if (command === 'pdmovie' || command === 'pdsearch') {
    ...
    try {
      const results = await publicDomainAPI.searchMovies(args, 8);
  ...
  if (pdMovieMatch && !body.includes('/movie')) {
    ...
    if (query.length > 2) {
      ...
      try {
        ...
        const formatted = publicDomainAPI.formatSearchResults(results, 5);
...
async function deliverAgentResult(ctx, result) {
  if (result && typeof result === 'object') {
    if (result.type === 'url') {
      ...
      return ctx.reply(MessageFormatter.reformatResponse(msg));
    }
    if (result.type === 'images' && Array.isArray(result.images)) {
      await ctx.reply(MessageFormatter.reformatResponse(`✅ Generated ${result.images.length} image(s) for: ${result.prompt || 'your prompt'}`));
      for (const image of result.images.slice(0, 10)) {
        const imageUrl = image.url || image.path || image;
        try {
          await ctx.replyWithPhoto(image.path ? { source: image.path } : imageUrl, { caption: MessageFormatter.reformatResponse(`🖼️ ${result.prompt || 'Generated image'}${image.seed ? `
Seed: ${image.seed}` : ''}`) });
        } catch (_error) {
    ...
    if (result.savedPath && await fs.pathExists(result.savedPath)) {
      await ctx.reply(MessageFormatter.reformatResponse(`✅ Done. Scrape saved: ${result.savedPath}
...
\`\`\``));
    ...
    if (result.path && await fs.pathExists(result.path)) {
      ...
      await ctx.reply(MessageFormatter.reformatResponse(`✅ Done. File created: ${result.path}`));
      if (isImage) {
    ...
    return ctx.reply(MessageFormatter.reformatResponse(`✅ ${jsonStr}`));
  }
  const textResult = String(result || 'Done').slice(0, 3500);
  return ctx.reply(MessageFormatter.reformatResponse(textResult));
}
...
bot.launch()
  .then(() => {
    ...
    console.log('📝 MessageFormatter loaded');
  })
  .catch((error) => {
